...
This is the second assignment for the course *ISE537: Statistical Models for Systems Analytics in Industrial Engineering*. The topic of this assignment is multiple linear regression, including:
...
The value of the residual associated with the last observation of the data set is **`r round(last_resid, 3)`**.
...
The approximate estimated variance of the error terms is **`r round(overall_sigma2, 2)`**.
...
When running the same model using the subset of rows with a `BMI` ≥ 30, there are **zero** regression coefficients that are significant at the $\alpha=0.01$ significance level.
...
fram_summary <- tibble("Model" = c("model_overall", "model_obese"),
                       "R2" = c(overall_r2, obese_r2),
                       "Variance" = c(overall_sigma2, obese_sigma2))
fram_summary
```

The amount of variability explained by the predictors is lower in the obese population compared to the overall population. This is because the $R^2$ for the obese model is `r round(obese_r2, 3)` whereas the $R^2$ for the full model is `r round(overall_r2, 3)`.

Additionally, the estimated variance of the error terms under the model for the overall population, `r round(overall_sigma2, 2)`, is lower than the estimated variance under the model for the obese population only, `r round(obese_sigma2, 2)`.

In the context of this question, the answer is **all of the above**.
...
# ensure first level is Pike
fish$Species <- relevel(fish$Species, "Pike")

# display statistical summary of cols
summary(fish)
```
...
Similar to `Body Height`, the relationship between `Weight` and `Total Length` appears approximately linear; as `Total Length` increases, so does `Weight`. In fact, there is likely a very strong correlation between `Total Length` and `Body Height` since both plots are almost identical.
...
The relationship between `Height` and `Weight` again appears to have multiple relationships, one that is more linear and another that is higher order. Overall, the relationship appears approximately linear with a positive slope, but we can confirm later.

There are several points around the center of the `Height` axis with `Weight` values above 1,500, which might be outlier points. Since they are near the mean of this predictor, this they might not have strong leverage.
...
The relationship between `Width` and `Weight` is different than what we have observed with the other predictors. This relationship looks the most linear, with a positive trend. We also note that since the `Width` , as well as the other quantitative predictors, cannot go below zero, it could be a clue that some sort of transformation is necessary.

There are several points with `Weight` values above 1,500, which might be outlier points. The point that is near the center of the $x$ data probably has lower leverage, but the others could have higher leverage.
...
As we suspected earlier, some of the predictors are highly correlated with one another: `Body Height`, `Total Length`, and `Diagonal Length`. Intuitively, this makes sense because these all sound like they are capturing the same information. We should not include all of these in our model in order to avoid multicolinearity.

Based on the entirely of our exploratory data analysis, it is reasonable to assume that the relationship between `Weight` and the predictors can be modeled using multiple linear regression. We just need to watch out for multicolinearity and potential higher order relationships when assessing model diagnostics and take corrective action when necessary.
...
-   The estimated coefficient for `Body Height` is -176.87, which means that the weight of a fish decreases by 176.87 grams for each 1 centimeter increase in body height, all other predictor variables constant. This seems very counterintuitive as bigger size generally means heavier weight. Likely, this is compensating for the estimated coefficients of other size variables, like `Total Length`, which is positive.

-   The estimated coefficient for `SpeciesParkki` is 62.585, which means that the weight of a fish increases by 62.585 grams if it is a *Parkki* instead of a *Pike*, all other predictors held constant. Of course, this is dependent on whatever the model considers the "base" fish, which in this case is *Pike*.
...
This is indeed the outlier we noted for a *Roach* fish with a weight of 1,700 grams. What row does this correspond to?
...
fish_no_outliers <- fish_train %>%
                      filter(!outlier)
model2 <- lm(Weight ~ ., fish_no_outliers)
summary(model2)
```

### 4. Checking Model Assumptions

We must now validate the residuals of our new model, `model2`, against several key assumptions:
...
We will start by computing the standardized residuals and creating a new column in `fish_no_outliers` .

```{r}
std_resid2 <- rstandard(model2)
fish_no_outliers$std_resid2 <- std_resid2
```

To assess the linearity assumption, let's plot these standardized residuals against each of the quantitative predictors. To make it easy to see all at once, let's display them on the same chart.
...
# create plot
ggplot(fish_melted) +
  geom_point(aes(Value, std_resid2), alpha=0.5) + 
  facet_wrap(~ Measure) +
  labs(title = "Linearity: model2")
```

There is curvature present in the `Body Height`, `Diagonal Length`, and `Total Length` plots, which is evidence that the linearity assumption does not hold. It is difficult to tell with `Width` and `Height` since they are on a different scale, so we look at those separately:
...
```{r}
ggplot(fish_no_outliers) +
  geom_point(aes(`Total Length`, std_resid3), alpha=0.5) +
  labs(title = "Linearity: model3", y = "Standardized Residual")
```

As we noted with the full model, this relationship has some curvature to it, suggesting that a transformation may be needed in order to satisfy the linearity assumption. The linearity assumption still does not hold.
...
The above QQ-plot confirms that the normality assumption, while still violated, is much better when compared against the full model.

### 7. Transformation

During diagnostics of `model2`, we noted that we need to address the nonlinear relationship present in the data. To do this, we will perform a Box-Cox transformation. First, we need to find the optimal $\lambda$:
...
The optimal lambda is `r round(lambda, 3)`, but we will round this to the nearest 0.5 by convention:
...
This new $\lambda$ suggests that we need to take the square root of our response variable. We will create a new column, `tWeight`, to capture this transformation.
...
```{r}
model4 <- lm(tWeight ~ Species + `Total Length`, 
             fish_no_outliers)
summary(model4)
```
...
```{r}
ggplot(fish_no_outliers) +
  geom_point(aes(`Total Length`, std_resid4), alpha=0.5) +
...
Similarly, this residual plot against the fitted values looks randomly distributed around zero, suggesting that the constant variance assumption holds for the transformed model.
...
Overall, there are a few predictions for `model3` that are fairly close to the true observed `Weight`, but also some values with large residuals. Additionally, there are actually negative predicted values for `Weight`. This is obviously impossible since the lower bound on the weight of a fish is zero.

When looking at the predictions of `model4`, they are much closer to the true observed `Weight`. Here there are no negative predictions, which is the result of our Box-Cox transformation of the response variable. This model has the higher $R^2$, but also appears to be much better at predicting as well. Let's confirm by calculating and comparing **M**ean **S**quared **P**rediction **E**rror (MSPE):
